...
from strava_plotter.__main__ import parse_rides, cluster_rides, plot_rides
...
def result(request):
    ...
    params["ids_to_skip"] = [
        id_to_skip.strip() for id_to_skip in params_from_form.pop("ids_to_skip")[0].split(";") if id_to_skip != ""
    ]
    params["alpha"] = float(params_from_form.pop("opacity")[0]) / 100
    params["activity_types"] = [
        activity_type for activity_type in params_from_form if params_from_form[activity_type] == ["on"]
    ]
